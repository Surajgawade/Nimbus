...
tinymce.PluginManager.add('bootstrap', function(editor, url) {
    ...
    var bsItems = [],
        ...
        jQuery.each(text, function(key, val) {
            bsText[key] = val;
            bsTip[key] = 'Insert/Edit Bootstrap ' + val;
        });
    if(menuType == 'buttongroup') {
        jQuery.each(text, function(key, val) {
             bsText[key] = '';
        ...
        if((e.keyCode == 13 || e.keyCode == 10)) {
            ...
            if(parents.length > 0) {
                ...
                if(e.altKey === true) {
                    ...
                    curElm = editor.selection.getRng().startContainer;
                    caretPos = editor.selection.getBookmark(curElm.textContent).rng.startOffset;
