    ...
    <div class="row">
      <div class="col-md-12">
         <div class="card" data-select2-id="9">
            ...
            <form name="update_child_category" id="update_child_category" method="post" action="javascript:void(0)" enctype="multipart/form-data">
                {{csrf_field()}}

                   <input type="hidden" name="hidden_uid" value="{{$child_category->id}}">
                    ...
                    <div class="col-sm-4 col-xs-4 col-lg-4  form-group">
                        ...
                        <select name="category_name" id="category_name" class="form-select">
                            ...
                            @foreach($category as $key=>$category_data)
                                <option value='{{$category_data->id}}'  <?php if($child_category->category_id == $category_data->id){echo "selected = 'selected'"; } else {echo ''; }?> >{{$category_data->category_name}}</option>
                            @endforeach
                        </select>
                      ...
                      <div class="col-sm-4 col-xs-4 col-lg-4  form-group">
                        ...
                        <select name="sub_category_name" id="sub_category_name" class="form-select">
                            ...
                            @foreach($sub_category as $key=>$sub_category_data)
                                <option value='{{$sub_category_data->id}}'  <?php if($child_category->sub_category_id == $sub_category_data->id){echo "selected = 'selected'"; } else {echo ''; }?>>{{$sub_category_data->sub_category_name}}</option>
                            @endforeach
                        </select>
                      ...
                      <div class="col-sm-4 col-xs-4 col-lg-4 form-group">
                        ...
                        <input type="text" name="child_category_name" id="child_category_name" value="{{$child_category->child_category_name}}" class="form-control">
                  ...
                  <div class="box-body">
                    <div class="col-md-6 col-sm-12">
                      <button update="submit" id="btn_update_child_category" name="btn_update_child_category" class="btn btn-primary">Update</button>
...
$("#update_child_category").validate({
    rules: {
      hidden_uid: {
        required: true
      },
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        ...
        url: '<?php echo ADMIN_SITE_URL . 'edit-child-category'; ?>',
        type: "POST",
        ...
        dataType:'json',
        beforeSend:function(){
          $('#btn_update_child_category').attr('disabled','disabled');
          $('#btn_update_child_category').html('Please Wait...');
        },
        complete:function(){
          $('#btn_update_child_category').attr('disabled',false);
          $('#btn_update_child_category').html('Submit');
        },
